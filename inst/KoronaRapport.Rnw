...
print(xtable::xtable(BeleggTab[fra:AntRader, ],
               ...
               caption='Andel av totalt antall senger (2019-tall) som opptas av pasienter hvor covid-19 er hovedårsak til siste opphold.'),
               include.rownames=FALSE)
@
\end{frame}
...
AntUt <- antallTidUtskrevne(RegData=PandemiOpph, tilgangsNivaa=rolle, valgtEnhet= valgtEnhet)$Tab
antEnh <- dim(AntUt)[2]
skalering <- ifelse(antEnh < 7, 1, 7/antEnh)
# print(
#   xtable::xtable(AntUt[(nrow(AntUt)-20):nrow(AntUt), ], digits=0,
#                method='compact', #align=c('l', rep('r', ncol(alderDIV))),
#                caption='Antall nye opphold (innlagte) med covid-19 som hovedårsak siste 20 dager,
#                samt totalt antall fra pandemistart til rapporteringstidspuktet.')
#       ,include.rownames = FALSE
...
xtable::xtable(RisikoTab, #digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
               caption=paste0('Risikofaktorer på  for
...
#print(
  ...
  FerdigeReg3mnd <- FerdigeRegTab(PandemiSiste3mnd)$Tab

#print(
  xtable::xtable(FerdigeReg3mnd, align=c('l','r','r','c','r','r'),
               #digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
               caption=paste0('Verdier på rapporteringstidspunktet for utskrevne pasienter innlagt henholdsvis
               før ', datoTil3mnd, ' (øverste tabell) og siste 3 måneder (nederste tabell).
               Både inklusjonsskjema og utskrivingsskjema er ferdigstilt for hele pasientforløpet.
