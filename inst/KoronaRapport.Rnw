...
#Data per opphold
#PandemiOpph <- KoronaPreprosesser(RegData = PandemiRaa, aggPers = 0)
...
#Kobler pandemi og beredskap: - SLETTES. DETTE ER GJORT I PREPROSESS
...
For resultater basert på personaggregerte data, er skjema aggregert til ett eller flere forløp per pasient. Hvis det er mer enn 3 måneder mellom to opphold,
regnes det som to forløp, ellers som ett.
Per i dag er det \Sexpr{antPasFlereForlAlleInn} pasienter som har mer enn ett forløp og \Sexpr{antPasFlereForl} av disse har mer enn ett forløp med Covid-19 som hovedårsak.
...
\begin{frame}[fragile] {Antall registrerte opphold per uke}
...
\begin{frame}[fragile] {Antall avsluttede forløp per uke}
...
print(xtable::xtable(AntUt[fra:nrow(AntUt),], digits=0, method='compact', #align=c('l', rep('r', ncol(alderDIV))),
                     caption='Antall avsluttede forløp (utskrevne) med covid-19 som hovedårsak siste tre måneder,
                     samt totalt antall fra pandemistart til rapporteringstidspuktet.
