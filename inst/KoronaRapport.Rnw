...
PandemiAlle <- Pandemi
Pandemi <-  KoronaUtvalg(RegData=PandemiAlle, aarsakInn = 1)
...
Dokumentet inneholder oppsummering av disse registreringene. Med mindre annet er angitt, er kun pasienter
...
xtable::xtable(InneliggPas, digits=0,
                              caption='Antall inneliggende pasienter (mangler utskrivingsdato) på hvert HF.')
# indNaa <- which(is.na(Pandemi$UtDato))
...
\begin{frame}[fragile] {Antall som ligger på sykehus av andre årsaker, men med Covid-19-smitte}
...
andelMennAndre <- paste0(sprintf('%.0f', 100*sum(PandemiAndre$erMann)/Ntot), ' \\%')
antDodAndre <- sum(PandemiAndre$StatusVedUtskriving==2, na.rm = T) #[indFerdigUt]


AntReg <- dim(PandemiAndre)[1]
...
For pasienter som har Covid-19, men er eller har vært lagt inn på sykehuset av andre årsaker, er
gjennomsnittsalderen \Sexpr{AlderGjsnAndre} år og \Sexpr{andelMennAndre} er menn. Det er \Sexpr{antDodAndre}
av \Sexpr{AntReg} som er døde.
...
<<'Belegg', results='asis'>>=
BeleggTab <- antallTidBelegg(RegData=Pandemi, aarsakInn=9)
...
xtable::xtable(FerdigeReg, #digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
