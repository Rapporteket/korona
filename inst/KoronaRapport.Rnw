...
%\usepackage{tabularx}
\usepackage{caption}
\captionsetup[figure]{font=tiny} %,labelfont=footnotesize}
\captionsetup[table]{font=tiny} %,labelfont=small}
...
#Telle pasienter med flere forløp, covid hovedårsak
    PasFlere <- PandemiOpphAlleInnlEgen %>% group_by(PasientID) %>%
      summarise(.groups = 'drop',
                InnNr0 = ifelse(Dato-min(Dato)>90, 2, 1))
    antPasFlereForlAlleInn <- sum(PasFlere$InnNr0>1)
...
uavhengig av om pasienten har reinnleggelser eller flere forløp. Per i dag er det \Sexpr{antPasFlereForlAlleInn} pasienter som har mer enn ett forløp og \Sexpr{antPasFlereForl} av disse har mer enn ett forløp med Covid-19 som hovedårsak.
...
xtable::xtable(RisikoTab, #digits=0, #method='compact', #
               ...
               caption=paste0(
                 'Risikofaktorer for ferdigstilte forløp fra pandemistart til ',
...
\begin{frame}[fragile] {Resultater for ferdigstilte forløp}
<<'Oppsum', results='asis'>>=

#Form status: 0 = Ingen	1 = Kladd	2 = Ferdigstilt	4 = Slettet	5 = Returnert
FerdigeReg <- FerdigeRegTab(PandemiUsiste3mnd, minN = 3)$Tab
...
print(
  ...
  FerdigeReg3mnd <- FerdigeRegTab(PandemiSiste3mnd, minN = 3)$Tab
