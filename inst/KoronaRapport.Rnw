...
andemiOpphAlleInnlEgen <-  KoronaUtvalg(RegData=PandemiOpph,
                         enhetsNivaa = enhetsNivaa, valgtEnhet=valgtEnhet)$RegData
PandemiOpph <-  KoronaUtvalg(RegData=PandemiAlleInnlEgen, aarsakInn = 2)$RegData
...
Hvis ikke annet er angitt, er \textit{\textbf{kun pasienter hvor Covid-19 er hovedårsak til  siste innleggelsen i forløpet}}, tatt med. Hvis pasienten er overført, skal minimum siste opphold ha Covid-19 som hovedårsak til innleggelsen.
...
dum <- KoronaFigAndeler(RegData = Pandemi, valgtVar = 'regForsinkelseInn', aarsakInn = 2,
...
print(xtable::xtable(RegHF, row.names=FALSE, digits=0,
                     caption='Totalt antall registrerte opphold i hver enhet.'),
      include.rownames = FALSE)
@
\end{frame}
...
\begin{frame}[fragile] {Antall registrerte opphold}
...
#                               valgtEnhet= valgtEnhet, HF = 1)$Tab_tidy
...
print(
  xtable::xtable(AntTab[(nrow(AntTab)-20):nrow(AntTab), ], digits=0,
               method='compact', #align=c('l', rep('r', ncol(alderDIV))),
               caption='Antall registrerte opphold med korona som hovedårsak for innleggelse siste 20 dager,
               samt totalt antall for hele pandemiperioden.')
      #include.rownames = FALSE,
      ,scalebox = skalering)
@
...
print(xtable::xtable(AntUt[fra:nrow(AntUt),], digits=0, method='compact', #align=c('l', rep('r', ncol(alderDIV))),
                              caption='Antall avsluttede opphold per dag siste 20 dager, samt totalt.'),
scalebox = skalering)
...
xtable::xtable(RisikoTab, #digits=0, #method='compact', #align=c('l', rep('r', ncol(alderDIV))),
                              caption='Pasientenes risikofaktorer ved innleggelse')
@
\end{frame}
