...
BeleggTab[AntRader,1] <- 'Dognplasser.2019'
...
print(
  xtable::xtable(AntTab[(nrow(AntTab)-20):nrow(AntTab), ], digits=0,
               ...
               caption='Antall nye innlagte med covid-19 som hovedårsak siste 20 dager,
...
\begin{frame}[fragile] {Antall avsluttede forløp per dag}
...
AntUt <- antallTidUtskrevne(RegData=PandemiLandet, tilgangsNivaa=rolle, valgtEnhet= valgtEnhet)$Tab #PandemiOpph
antEnh <- dim(AntUt)[2]
#skalering <- ifelse(antEnh < 6, 1, 6/antEnh)
bredde <- 6/ncol(AntUt)
...
print(xtable::xtable(AntUt[fra:nrow(AntUt),], digits=0, method='compact', #align=c('l', rep('r', ncol(alderDIV))),
                     caption='Antall avsluttede forløp (utskrevne) med covid-19 som hovedårsak siste 20 dager,
                     samt totalt antall fra pandemistart til rapporteringstidspuktet.
