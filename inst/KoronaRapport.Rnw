...
library(xtable)
library(korona)
forHvem <- ifelse(egenEnhet=='Alle', 'hele landet', egenEnhet)
...
\title[Pandemiregister \\\today]{Tittel} %\\\today}
...
#Innparametre:  egetEnhetsNivaa, egenEnhet, rolle?
...
PandemiLandet <- KoronaUtvalg(RegData=Pandemi, aarsakInn = 1)$RegData
PandemiAlleInnlEgen <-  KoronaUtvalg(RegData=Pandemi,
                         enhetsNivaa = egetEnhetsNivaa, valgtEnhet=egenEnhet)$RegData
Pandemi <-  KoronaUtvalg(RegData=PandemiAlleInnlEgen, aarsakInn = 1)$RegData
...
Hvis ikke annet er angitt, er \textit{kun pasienter hvor Covid-19 er hovedårsak til innleggelsen}, tatt med.
...
AntTab <- antallTidEnhTab(RegData=PandemiLandet, tilgangsNivaa=rolle, #nivå avgjort av rolle
                              valgtEnhet= egenEnhet)$Tab_tidy

#AntTab <- table(Pandemi$InnDag, Pandemi$RHF)
#AntTab <- addmargins(AntRegRHF)
...
<<'RegHF', results='asis'>>=
#nivaa1 <- egetEnhetsNivaa
nivaa2 <- ifelse(rolle=='LU', 'ShNavn', 'HF')
RegHF <- if (rolle == 'LU') {Pandemi %>% dplyr::group_by(RHF, HF) %>% dplyr::summarise(Antall = n())
  } else {Pandemi %>% dplyr::group_by(HF, ShNavn) %>% dplyr::summarise(Antall = n())}
...
xtable::xtable(InneliggPas, digits=0,
               caption='Antall inneliggende pasienter (mangler utskrivingsdato) på hver enhet,
               samt totalt.')
@
\end{frame}
...
# Antall som ligger på sykehus nå
PandemiAndre <- KoronaUtvalg(RegData = PandemiAlleInnlEgen, aarsakInn = 2)$RegData
...
BeleggTab <- as.data.frame(antallTidBelegg(RegData=Pandemi)$belegg_anslag_txt)
...
#Form status: 0 = Ingen	1 = Kladd	2 = Ferdigstilt	4 = Slettet	5 = Returnert
AntKladdShus <- table(#nivå avgjort av rolle$ShNavn[which(#nivå avgjort av rolle$FormStatus==1)], dnn= 'Skjema i kladd')
